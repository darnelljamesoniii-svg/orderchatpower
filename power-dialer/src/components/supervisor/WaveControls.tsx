      ...
      return onSnapshot(ref, snap => {
        if (snap.exists()) {
          setWaves(prev => {
            ...
            return [...next, { id: snap.id, ...snap.data() } as CampaignWave].sort((a, b) => a.id.localeCompare(b.id));
          });
        } else {
          // Seed defaults
          import('@/lib/firebase-admin').then(() => {}).catch(() => {});
        }
      });
    });
        ...
        {waves.map(wave => (
          <div key={wave.id} className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => toggle(wave)} className="transition-colors">
                {wave.isActive
